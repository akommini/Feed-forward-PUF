...
for($deviceId=$startDeviceId;$deviceId<($numOfChips+$startDeviceId);$deviceId++)
	...
	for($currIteration=1;$currIteration<=$numOfIterations;$currIteration++)
	{
		...
		for($iteration=$startIteration;$iteration<=$endIteration;$iteration++)
			...
			open inputNetlist, "<","/home/akommini/658proj/puf_net.sp";
			...
			{
				print netlistFile ".tran 10ps ",5*$period*($numOfCycles) ,"ns UIC\n";
				...
				#{			
				#	print netlistFile ".MEASURE TRAN tdlayS",$tmpStage,"a",$i, " TRIG V(sdla",$tmpStage,") VAL = $half_val RISE = 1 TD=",$currDelay ,"ns TARG V(sdlb",$tmpStage,") VAL = $half_val RISE = 1\n";
				#}
				print netlistFile ".MEASURE TRAN PowEva INTEG I(vsupply) From=0ns TO=",$period-0.01,"ns\n";
	#			print netlistFile ".MEASURE TRAN PowEva1Half",$i, " INTEG I(vsupply) From=",$currDelay-0.1, "ns TO=",$currDelay+0.25,"ns\n";
				...
				print netlistFile".MEASURE Etot PARAM='$voltage*PowEva'\n";
				print netlistFile ".MEASURE TRAN outv",$i, " AVG V(out) FROM=9ns TO=9.01ns\n";
				#print netlistFile ".MEASURE TRAN outv2",$i, " AVG V(Sa30) FROM=", $currDelay,"ns TO=", $currDelay+0.0001,"ns\n";
				#print netlistFile ".MEASURE TRAN outv1",$i, " AVG V(Sb30) FROM=", $currDelay,"ns TO=", $currDelay+0.0001,"ns\n";
				#print netlistFile ".MEASURE TRAN ya",$i, " AVG V(ya) FROM=", $currDelay,"ns TO=", $currDelay+0.0001,"ns\n";
				print netlistFile ".MEASURE TRAN dlypd",$i, " TRIG V(start1) VAL = $half_val RISE = 1 TD=",$currDelay-0.5 ,"ns TARG V(net03559) VAL = $half_val RISE = 1\n";
				print netlistFile ".MEASURE TRAN dlystg64 TRIG V(Sa64) VAL = $half_val RISE = 1 TD=",$currDelay ,"ns TARG V(Sb64) VAL = $half_val RISE = 1\n";
				print netlistFile ".MEASURE TRAN dypwr AVG POWER from=2ns to=4ns\n";
			...
			$tmpStrNew = "Results/Reliable_arbpuf/Chip";
                     $tmpStr2 = $tmpStrNew.$deviceId."/"."run".$currIteration."/"."voltage".$ARGV[3]."/"."result_row1_temp.txt";
		     $stvolt= $tmpStrNew.$deviceId."/"."run".$currIteration."/"."voltage".$ARGV[3]."/"."result_row2_temp.txt";
			#system("awk '{ print $3 }' $tmpStr1 > $tmpStr2");
			system("sed '5 !d' $tmpStr1 >> $tmpStr2");
			system("sed '6 !d' $tmpStr1 >> $stvolt");
			#system("awk '{ print $3 }' $tmpStr2 > $stvolt");
